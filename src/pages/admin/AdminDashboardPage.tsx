...
import type { InvitedGuestRow } from '../../lib/eventCatalog'
...
export function AdminDashboardPage() {
  ...
  const [guests, setGuests] = useState<InvitedGuestRow[]>([])
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setGuests((gu.data ?? []) as InvitedGuestRow[])
  ...
  const startEditGuest = (g: InvitedGuestRow) => {
  ...
  const clearAllRsvpResponses = async () => {
    ...
    const ok = window.confirm(
      `Apagar todas as ${total} confirmação(ões) salvas no banco?\n\n` +
        'Isso limpa o relatório RSVP (incluindo dados de teste). A lista de convidados não é alterada.\n\n' +
      ...
      {loading ? (
        ...
        <>
          <section className="admin-section">
            <h2 className="admin-h2">Lista de convidados</h2>
            <p className="admin-section__hint">
              Nomes que aparecem na página de confirmação de presença.
            </p>

            <SpreadsheetUpload sb={sb} existingIds={guests.map((g) => g.id)} onDone={reload} />
          ...
          <section className="admin-section">
            <h2 className="admin-h2">Relatório de confirmações</h2>
            <p className="admin-section__hint">
              Uma linha por convidado, com a última resposta registrada.
            </p>
              ...
              <>
                <div className="admin-rsvp-summary" aria-live="polite">
                  ...
                  <span className="admin-rsvp-summary__muted">
                    Confirmados: <strong>{rsvpSummary.headsConfirmed}</strong>
                  </span>
                </div>
